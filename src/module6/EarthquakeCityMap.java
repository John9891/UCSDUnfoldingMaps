...
public class EarthquakeCityMap extends PApplet {
	...
	public void setup() {		
	 ...
	 // create markers from features
	 	for(PointFeature feature : featuresAirports) {
	 		...
	 		int altitude = Integer.parseInt(m.getStringProperty("altitude"));	 		
	 		if(altitude>10000){
	...
	/**This method sorts an array of EarthquakeMarkers in reverse order of their magnitude (highest to lowest)
	 * and prints out all of the earthquakes
	 */
	private void sortAndPrint(int numToPrint){
		
		EarthquakeMarker arrayMarkers [] = new EarthquakeMarker[quakeMarkers.size()];
		quakeMarkers.toArray(arrayMarkers);
		
		int indActual = 0;	
		
		for(int i=1; i<arrayMarkers.length; i++){
			
			indActual = i;			
			while(indActual>0 && arrayMarkers[indActual].compareTo(arrayMarkers[indActual-1])>0){				
				swap(arrayMarkers, indActual, indActual-1);
				indActual = indActual - 1;					
			}			
			
		}
		
		if(numToPrint > arrayMarkers.length){
			numToPrint = arrayMarkers.length;			
		}
		for(int i=0; i<numToPrint; i++){
			System.out.println(arrayMarkers[i].toString());
		}		
	}
